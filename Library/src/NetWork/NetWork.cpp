...
NET_TRY
	...
	NET_LOG("添加%d个网络工作线程",nThreadCount);
...
void NetWork::LogDebugInfo()
{
	NET_LOG("==========网络运行状态==========");
	{
		NET_LOG("当前ServerCon数量:[%d]",S_NetConnection::s_count);
		NET_LOG("当前ClientCon数量:[%d]",C_NetConnection::s_count);
