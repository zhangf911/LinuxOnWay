...
NET_TRY
	...
	if (_error)
	{
		if (NetError_EOF != _error.value())
			NET_ERROR("[%d]接收数据失败,错误代码:[%d-%s]",m_id,_error.value(),_error.message().c_str());	
...
NET_TRY
	...
	if (_error) 
	{
		...
		NET_ERROR("[%d]发送数据失败,错误代码:[%d-%s]",m_id,_error.value(),_error.message().c_str());
...
NET_TRY
	...
	NET_LOG("向[%s:%d]发起连接",_address.to_string().c_str(),nPort);
...
NET_TRY
	if (_error)
	{
		NET_ERROR("无法连接到服务器，错误代码:[%d-%s]",_error.value(),_error.message().c_str());
		...
		//自动重连
		NET_LOG("自动重新连接到服务器",_error.value());
	...
	{
		NET_LOG("连接到服务器，正在建立Connection[%s]",m_socket.local_endpoint().address().to_string().c_str());
...
NET_TRY
	...
	NET_LOG("向[%s:%d]发起重连",m_remoteAddress.address().to_string().c_str(),m_remoteAddress.port());
	return true;
NET_CATCH
	return false;
}
...
NET_TRY
	if (_error)
	{
		NET_ERROR("建立连接的时候失败了,接收建立协议失败,错误代码:[%d-%s]",_error.value(),_error.message().c_str());
		m_ConnectedHandler(_error.value());
	}
	else
	{
		...
		if (pPacket)
		{
			...
			m_state = State_Connected;
			NET_LOG("建立连接完成,[%d] = [%s:%d]",m_id,m_remoteAddress.address().to_string().c_str(),m_remoteAddress.port());
			_appendreceive();
...
NET_TRY
	if (_error)
	{
		NET_ERROR("建立连接的时候失败了,发送建立协议失败,错误代码:[%d-%s]",_error.value(),_error.message().c_str());
		//自动重连
		m_ConnectedHandler(_error.value());
...
NET_TRY
	if (_error)
	{
		NET_ERROR("[%d]Accept失败,错误代码[%d-%s]",m_id,_error.value(),_error.message().c_str());
		m_socket.close();
		m_state = State_Stopped;
	}
	else
	{
		...
		NET_LOG("[%d]Accpet成功，正在建立Connection[%s]",_id,m_remoteAddress.address().to_string().c_str());
#ifdef ENABLE_INTERNAL_CONNECTION
		SetupConnection();
#else
		m_state = State_Connected;
...
NET_TRY
	if (_error)
	{
		NET_ERROR("[%d]建立连接的时候失败了,接收建立协议失败,错误代码:[%d-%s]",m_id,_error.value(),_error.message().c_str());
		m_socket.close();
		m_state = State_Stopped;
	}
	else
		...
		if (pPacket)
		{
			...
			if (bReConnect)
			{
				...
				else
				{
					...
					NET_LOG("建立重连连接完成 [%d] = [%s:%d]",m_id,m_remoteAddress.address().to_string().c_str(),m_remoteAddress.port());
				}
			}
			else
			{
...
NET_TRY
	if (_error)
	{
		NET_ERROR("[%d]建立连接的时候失败了,发送建立协议失败,错误代码:[%d-%s]",m_id,_error.value(),_error.message().c_str());
		m_socket.close();
		m_state = State_Stopped;
	}
	else
